...
def gating_widget():
    ...
    def _solve_and_plot(m0=0.0, alpha=1.0, beta=1.0):
        ...
        ax.axhline(
            ...
            linestyle="--",
            linewidth=1.5,
            label=rf"$m_\infty = {m_inf:.3f}$",
        )
        # After the axhline for m_inf, add:
        if np.isfinite(tau_m):
            ax.axvline(
                tau_m,
                color="C2",
                linestyle=":",
                linewidth=1.5,
                label=rf"$\tau_m = {tau_m:.3f}$ ms",
            )
...
def voltage_dependence_widget():
    ...
    interact(
        ...
        d_alpha=FloatSlider(
            value=15, min=1, max=50, step=1, description="d_α", continuous_update=False
        ),
        V_beta=FloatSlider(
            value=-40,
            min=-100,
...
def fixed_conductance_widget():
    ...
    def _solve_and_plot(V0=-60, I_amp=1.0, g_Na=0.0, g_K=0.2):
        def rhs(t, y):
            V = y[0]
        ...
        ax.axhline(
            E_Na, color="C3", linestyle="--", alpha=0.5, label=f"$E_{{Na}}$ = {E_Na} mV"
        )
        ax.axhline(
            E_K, color="C4", linestyle="--", alpha=0.5, label=f"$E_{{K}}$ = {E_K} mV"
        )
        ax.set_xlabel("Time (ms)")
        ax.set_ylabel("V (mV)")
...
def voltage_gated_conductance_widget():
    ...
    g_Na = 0.8  # µS
    g_K = 0.1  # µS
    Vs = -20.0  # half-activation voltage (mV)
    d = 10.0  # slope parameter (mV)
    ...
    def _solve_and_plot(V0=-65, I_amp=0.0):
        def rhs(t, y):
            V = y[0]
            I_app = I_amp if t < 1 else 0.0
            m = _m_inf(V)
...
def hh_explorer_widget():
    ...
    def _solve_and_plot(
        g_Na=120, g_K=36, g_L=0.3, I_amp=10, stim_start=2, stim_dur=1, V0=-65
    ):
        ...
        t_span = (0, stim_start + stim_dur + 10)
        t_eval = np.linspace(*t_span, int(100 * (t_span[1])))
        ...
        axs[0].axvspan(stim_start, stim_end, alpha=0.1, color="yellow")
        axs[0].set_xlabel("Time (ms)")
        axs[0].set_ylabel("V (mV)")
    ...
    interact(
        ...
        g_K=FloatSlider(
            value=36, min=0, max=100, step=1, description="ḡ_K", continuous_update=False
        ),
        g_L=FloatSlider(
            value=0.3,
            min=0,
        ...
        stim_dur=FloatSlider(
            value=1,
            min=0.1,
            max=25,
            step=0.1,
            description="Stim dur",
            continuous_update=False,
        ),
        V0=FloatSlider(
            value=-65,
