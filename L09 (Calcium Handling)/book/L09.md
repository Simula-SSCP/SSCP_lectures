# L9 — Quantitative Aspects of Calcium Handling

In this lecture we will start shifting our focus from the membrane of the cell, to what happens inside the cell. Here we will mainly discuss intracellular _calcium handling_. This is an important topic because calcium is what couples electrical activation of the cell to contraction, a process typically called excitation-contraction (EC) coupling.

We will leave contraction and force production as a topic for this afternoon, and for now we focus on understanding how calcium enters and leaves the cytosol during an action potential. As usual this is quite a complex topic, and there are many details. We try to focus on giving you an overview and point out the most important actors involved in calcium handling and how to model these.

The main topics we cover in this lecture are as following

- [Excitation-Contraction coupling](#ECC)
- [Calcium Cycling](#cycling)
- [The SERCA Pump](#serca)
- [The Na-Ca Exchanger](#NCX)
- [Regulation of EC coupling](#regulation)

<a id='ECC'></a>

# Excitation-Contraction Coupling in the Cell

Over 200 years ago, Luigi Galvani reported that the leg muscles of dead frogs would twitch if he created an electrical spark. This was the beginning of the study of _excitation-contraction coupling_, the process where electrical activation of a muscle cell causes it to contract.

We have so far studied the cellular action potential in detail, and have seen how gated ion channels in the cell membrane are responsible for producing these spontaneous electrical signals. We will now try to understand how depolarization of the membrane can cause the cell to contract.

The action potential occurs mainly at the cell membrane, but contraction occurs in the myofilaments within the cell. For exciation and contraction to be coupled, we therefore need some process which can transport the signal from the depolarizing cell membrane, into the cytosol and bind to the myofilaments. It turns out that this signal is mediated by diffusing calcium ions (Ca$^{2+}).

### Calcium as a signaling molecule

The rapid changes in the membrane potential of the cell during an action potential occurs due to electrically charged ions (mainly Na$^+$, K$^+$ and Ca$^{2+}$) passing through ion channels in the membrane. For sodium and potassium, these fluxes have relatively small impacts on the intracellular ion concentrations, and these shift at most a few percent. For calcium, however, the relative change in concentration can enormous, with the cytosolic calcium levels increasing by as much as a thousandfold on the order of tens of milliseconds. This is not because calcium fluxes are much larger in magnitude than sodium and potassium, but rather that the diastolic calcium levels are _incredibly_ low, typically as low as ~100 nM, meaning the relative changes in concentrations become much larger.

The rapid increase in calcium concentration in the cell is known as the calcium _transient_. This is because calcium levels decay back to resting levels during repolarization, and so the increased calcium levels occur only _transiently_. Put simply, we can say that this calcium transient is what triggers contraction of the cell. Precisely because the transient is so strong and occurs only during systole, it is a very clear signal in the cell. In comparison, sodium and potassium levels have such small relative shifts that they have poor signal-to-noise ratio.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/ca_transients.png" width=500></center>
<center><div style="max-width:750px; word-wrap:break-word;"><b>Figure 1:</b> On the top you can see a train of three action potentials. On the bottom you can see the corresponding cytosolic calcium levels. During the depolarizing phase we see a rapid and drastic increase in cytosolic calcium over tens of milliseconds. During the repolarizing and resting phase of the AP, this calcium decays and returns to diastolic levels. Because the increase in calcium is only temporary, we typically call this signal the <i>calcium transient</i>.</div></center>
    
<p style="margin-bottom:1cm;"></p>
<center><img src="fig/skeletal_EC2.png" width=600></center>
<center><div style="max-width:750px; word-wrap:break-word;"><b>Figure 1b:</b> The action potential is what drives the calcium transient, which in turn causes contraction of the cell. This is why we say calcium is the mediator of excitation-contraction coupling.</div></center>
<p style="margin-bottom:1cm;"></p>

In addition to being a signal for contraction, calcium is a signal for a large number of process in the cell, both short term and long term. In the short term, elevated calcium levels can drive the activity of kinases, phosphatases, ion channels, exchangers and transporters. In the long term, elevated levels of calcium can be responsible for function, growth, gene expression, differentiation, remodeling and development of cells.

Put simply, calcium makes a lot of stuff happen. In chronic heart disease, calcium handling is often found to be disrupted. In addition, calcium handling is quite different across mammalian species, and understanding these differences can be crucial when using animal models or develop drugs. While calcium is important for a large number of signaling pathways, what we mainly want to focus on right now is how the calcium transient itself is formed, and how it decays back to resting levels.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/calmodulin.png" width=500></center>

<center><div style="max-width:800px; word-wrap:break-word;"><b>Figure 2: Examples of calcium signaling molecules</b> <br>
    <b>Left:</b>Calcium-modulated protein (calmodulin or CaM) is found in the cytosol. When there is no calcium bound to it, it is inactive. When calcium binds to CaM it in turn binds to other molecules in the cell triggering long chains of other processes.<br>
    <b>Right:</b> Calcium/calmodulin dependent protein kinase II (CaMKII) are macromolecules in cardiomyocytes activated by calmodulin. Kinases are enzymes that drive <i>phosphorylation</i> which in turn alters the activity of other processes in the cell.</b> 
</div></center>
<p style="margin-bottom:1cm;"></p>

### Force as a function of the calcium transient

Contraction of the cell occurs when there is enough free calcium in the cytosol, which can then bind to organelles known as _myofilaments_, causing contraction. The mechanism of force production in the myofilaments is described by _sliding filament theory_, which is the topic of the following lectures. For now we want to simple state that free calcium causes contraction, and that contraction ends once calcium levels drop back down.

Note, however, that contraction is _not_ an all-or-nothing response to calcium. Rather, the strength of contraction is a direct function of the _magnitude_ of the calcium transient. We can model this using the Hill equation, using a Hill coefficient of $n=4$ and a half-saturation point of $K_{1/2} = 600$ nM free calcium. If you recall our discussion on the Hill equation, a coefficient of 4 means force production will be sigmoidal, with a steep dependence on the magnitude of the calcium transient. This is shown in Figure 4 below (the smaller inset curve).

Figure 4 contains two graphs, and the distinction between the two is important. The smaller inset graph shows force production as a function of _free_ cytosolic calcium, i.e, the actual concentration $\rm [Ca^{2+}]_i$. The larger graph shows instead how much calcium we have to _add_ to the cytosol to trigger contraction.
The difference from the two comes mainly from the fact that the cytosol contains a large number of calcium buffers, that will effectively bind and lower the free calcium levels following any Ca$^{2+}$-influx. We therefore need to add a lot more calcium than what would naively think, to properly trigger contraction.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/force_calcium_curve.png" width=500></center>
<center><div style="max-width:750px; word-wrap:break-word;"><b>Figure 3:</b>Contractile force produced as a function of cytosolic calcium levels. Note that contraction is not an all-or-nothing response, but has a graded and smooth dependence on the amount of calcium in the cytosol. This means that if the cell produces a bigger calcium transient, it will result in a stronger production of force.
       <br>Figure taken from [Donald M. Bers Circulation Research. 2000;87:275-281] </div></center>
<p style="margin-bottom:1cm;"></p>

## Intracellular Structures of the Cardiomyocyte

To better understand how the calcium transient is formed, it helps to have an overview of the most important structures of the cell. We therefore show a simplified figure of these

<center><img src="fig/cardiomyocyte.png" width=600></center>
<center><div style="max-width:750px; word-wrap:break-word;"><b>Figure 4:</b>Schematic of some of the most important calcium handling structures inside the myocyte.
       <br>Figure taken from [Zipes and Jalife, 2013]</div></center>

Let us give a brief explanation of the different structures shown in the figure:

- **The Cell Membrane**, also known as the _sarcolemma_. During an action potential calcium enters the membrane through calcium channels, primarily the L-type channel. Calcium efflux through the membrane is primarily driven by the Na$^{+}$/Ca$^{2+}$-exchanger (NCX) and the plasma membrane Ca$^{2+}$-pump (PMCA).
- **T-tubules** (transverse tubules) are invaginations of the membrane into the cell, effectively giving the cell a much larger surface area and ensuring most of the internal volume of the cell has a short distance to the membrane.
- **Myofilaments** contract and produce force and are activated by cytosolic calcium binding to them.
- **The mitochondrion** is the powerhouse of the cell and produces the actual energy required for contraction in the form of ATP. In addition the mitochondria does absorb and extrude a small amount of Ca$^{2+}$ but these quantities are small and usually ignored in models.
- **The sarcoplasmic reticulum (SR):** The SR is an internal calcium storage inside the cell. The SR consists of a lipid bilayer membrane impermeable to Ca$^{2+}$, just like the external cell membrane. The SR membrane also contains ion channels and pumps for calcium.
- **Dyads** are regions where the SR membrane and the external membrane lie close to each other, and these regions are especially important for calcium release, as we will explain shortly.

## The Calcium Transient, step by step

As mentioned, a calcium transient is triggered when the cell fires an action potential. Let us step through the whole process, starting at rest, then seeing how the transient increases and decays back to rest.

#### Step 1: Calcium Influx

When the cell is at rest, the cytosolic calcium levels are very low. The calcium levels outside the cell, and inside the SR (the intracellular calcium stores) are much, much higher. However, no ion channels in these membranes are open when the cell is at rest, so the cytosolic calcium remains low.

When the action potential starts the the membrane potential depolarizes, L-type calcium channels (LTCC) in the cell membrane open and calcium starts to flow into the cell.

<center><img src="fig/ecc_ltcc.png" width=600></center>
<center><div style="max-width:750px; word-wrap:break-word;">
<b>Figure 5a:</b>
At the start of the AP, calcium starts entering through the L-type Ca$^{2+}$ channels.
</div></center>

#### Step 2: Calcium-Induced Calcium Release

The cell contains intracellular calcium stores called the SR and the SR membrane has its own calcium channels that can open and release Ca$^{2+}$ into the cytosol. These channels are known as ryanodine recepetors (RyR). However, RyRs cannot sense the depolarizing membrane of the cell, because they are not a part of the membrane. Instead they are triggered to open in response to calcium binding to the cytosolic side of the receptor.

When the cell fires an AP, calcium enters through the L-type channels and bind to the RyRs, triggering a secondary release of calcium from the SR. This process is known as _calcium induced calcium release_ (CICR). Under normal conditions, the calcium flux from the SR through RyR is much larger than the influx from outside the cell through the L-type channel and it is this secondary release that can ensure a fast and large calcium transient.

To ensure a strong and robust CICR mechanism most L-type and RyR channels are located close to each other in microdomains called dyads. The membrane also has t-tubules that burrow into the cell allowing the cell to have dyads distributed throughout the cell, not just at the periphery. The calcium released from the SR enters into the dyad, but then diffuses into the rest of the cytosol over time.

<center><img src="fig/ecc_cicr.png" width=600></center>
<center><div style="max-width:600px; word-wrap:break-word;">
<b>Figure 5b:</b>
Following influx of Ca$^{2+}$ through L-type channels, a secondary calcium release from the SR through RyR channels is triggered.
</div></center>

#### Step 3: Diffusion and Contraction

A large amount of calcium is entering into the cytosol from both outside the cell and from the SR calcium store. This influx is located primarily in cardiac dyads, but calcium is free to diffuse into the bulk cytosol.

As calcium diffuses and spreads it starts binding to a plethora of targets, including myofilaments, triggering contraction of the cell. These binding reactions are dynamic, meaning calcium is binding and releasing continuously. The higher the cytosolic calcium levels rise, the stronger contraction becomes.

<center><img src="fig/ecc_buffering.png" width=600></center>
<center><div style="max-width:600px; word-wrap:break-word;">
<b>Figure 5c:</b>
As calcium diffuses out of the dyads into the bulk cytosol it starts binding to myofilaments, causing them to contract.
</div></center>

#### Step 4: Decay back to baseline

As the action potential repolarizes and returns to resting levels calcium has to be removed from the cytosol to ensure that the trigger for activation is removed and the myofilaments can relax long enough for the heart to refill.

To remove calcium from the cytosol, it must be pushed against its concentration gradient, i.e., active transport is needed. Calcium is therefore removed from the cytosol through exchangers and pumps. The most important of these are the SERCA pump and the Na/Ca exchanger.

The SERCA pump uses ATP to pump calcium back into the SR from the cytosol. This both reduces the cytosolic calcium, and replenished the SR calcium stores. The Na/Ca exchanger (NCX) pumps calcium ions through the external membrane, out of the cell, by coupling it to an influx of sodium. Sodium wants to enter the cell at rest, and so by coupling these two processes we can pump calcium out, by letting sodium in.

Another pump in the sarcolemma is the plasma membrane calcium ATPase (PMCA), which also uses ATP to push calcium out. When comparing the two main sarcolemma calcium efflux by capacity, NCX is much more important, moving much more calcium than PMCA.

<center><img src="fig/ecc_decay.png" width=600></center>
<center><div style="max-width:600px; word-wrap:break-word;">
<b>Figure 5d:</b>
At the end of the action potential, calcium is removed from the cytosol again to ensure relaxation of the cell. This happens through the SERCA and PMCA pumps and the $\rm Na^+/Ca^{2+}$-exchanger (NCX).
</div></center>

### Complete picture

We now combine all these processes into a single image. When reading papers or textbooks or attending talks you will often see images such as these. They can be a bit chaotic and overwhemling, but by breaking down the individual components and process they become more manageable. It is important to note that these images often do not show "snapshots" where everything is occurring simultaneously, rather they show the sum of a whole action potential.

<center><img src="fig/ecc_complete.png" width=600></center>
<center><div style="max-width:600px; word-wrap:break-word;">
<b>Figure 5e:</b>
A schematic of the whole EC coupling process.</div></center>

<a id="cycling"></a>

## Calcium Cycling

The process outlined above repeats for every single beat of the heart. For every action potential there is calcium influx, triggering contraction, and then a decay back to baseline as calcium is pumped back out of the cytosol. This process is sometimes called _calcium cycling_, to emphasize the cyclical manner in which calcium moves back and forth between the different domains. There are some subtle balances in this cycling that is very important for how the cell is regulated.

Recall now that the main actors of calcium _influx_ into the cytosol are:

- The L-type calcium channels, which drive calcium from the extracellular space into the cytosol
- The RyR calcium release channels, which drive calcium from the SR into the cytosol

The main actors of calcium _efflux_ are:

- The NCX, which pumps calcium out of the cell
- The SERCA, which pumps calcium into the SR

Because all calcium that enters the cell enters into the same domain, NCX and SERCA effectively _compete_ for the same calcium when they pump it back out. If one is more active, then the other will get less. However, the calcium that is pumped by the SERCA pump does not leave the cell entirely, but is rather stored in the SR. This stored calcium can then help strengthen the next calcium transient.

If _more_ calcium is pumped by SERCA than is released by RyR, then the amount of calcium stored in the SR, increases. This is called the _SR load_. If the SR load is higher, then RyR would release more calcium in the next beat, giving stronger transients. If _less_ calcium is pumped by SERCA, then the SR load would _decrease_.

Because the heart is continuously pumping, it is clear that the SR load cannot decrease or increase forever. Instead, the system will spontaneously tend towards a balance where the amount of calcium released from the SR is the same amount that is pumped back into the SR by SERCA. The same must then necessarily be through for the L-type and NCX. So unless there are some shift in conditions, and we are at _steady-cycle_, then for each beat of have

$$\int J_{\rm LTCC} \ {\rm d}t = \int J_{\rm NCX} \ {\rm d}t, \qquad  \int J_{\rm RyR} \ {\rm d}t = \int J_{\rm SERCA} \ {\rm d}t.$$

Thus, even though the calcium entering and leaving the cell is mixed in the cytosol, there are effectively two smaller calcium cycles that interact.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/calcium_cycling.png" width=400></center>
<center><div style="max-width:600px; word-wrap:break-word;">
<b>Figure 6:</b>
For each beat, calcium is cycled into and out of the cell through the sarcolemma by the LTCC and NCX, and released and taken up into the SR by RyR and SERCA. The two "cycles" will tend to balance out over time, meaning the amount of calcium released from the SR, is the same amount that is pumped back in.</div></center>
<p style="margin-bottom:1cm;"></p>

The balance between external and internal calcium cycling might seem subtle, but it has a large impact on how this system behaves. As an example we can compare the balance of NCX and SERCA in different species.

In species with a fast heart rhythm and short action potentials, such as rat, there is less time for influx of calcium through L-type channels, and so they need a stronger internal release of calcium to compensate. This means that SERCA has to be stronger compared to NCX in these species. In species with slower heart rhythms and longer action potentials, the LTCC stay open longer, allwoing more calcium to enters. As a result, these species are less reliant on SR release, and so NCX can be stronger compared to SERCA.

<p style="margin-bottom:1cm;"></p>

<center><img src="fig/cycling_fast_vs_slow.png" width=800></center>
<center><div style="max-width:800px; word-wrap:break-word;">
<b>Figure 7:</b>
The relative importance of LTCC/NCX vs RyR/SERCA can look very different in different species. On the left are species with short action potentials (and high heart rates), such as rats and mice. These rely strongly on Ca$^{2+}$ release from the SR and therefore needs SERCA to be strong. On the right we have species with longer action potentials (and lower heart rates), like humans. Here we are less reliant on SR Ca$^{2+}$ and SERCA is therefore weaker. <br>
Figure taken from <a href=http://rsta.royalsocietypublishing.org/content/367/1896/2181.short>Koivumäki et al, 2009.</a></div></center>

<p style="margin-bottom:1cm;"></p>

<center><img src="fig/cycling_fast_vs_slow_bers.png" width=300><center>
<center><div style="max-width:700px; word-wrap:break-word;">
<b>Figure 8:</b>
In this experiment, calcium leaving the cytosol is integrated over time. In rats, a species with fast heart rhythms, most calcium is cycled into the SR. By comparison, rabbits have slower heart rhythms and more calcium therefore leaves the cell entirely through NCX for each beat. Finally is a rabbit with heart failure, in which case the balance is disrupted, pushing it even more towards NCX.<br>
Figure taken from [Donald M. Bers Circulation Research. 2000;87:275-281]</div></center>

<p style="margin-bottom:1cm;"></p>

The balance between internal and external calcium cycling is not only different between different species, but also between different cell types. It can also change with maturation and disease. Heart failure for instance, is often associated with a decrease in SERCA function, as shown in Figure 8C. From what you have learned so far about calcium cycling, what do you think would be the consequence of downregulated SERCA-function?

### A simple model of calcium cycling

We will now turn to an interactive exercise, where you will implement a numerical model of calcium cycling. To make a proper model of calcium handling in the cardiomyocyte we need a full action potential model, and interactions get quite heavy very fast. As we would like a simpler model for this exercise, we instead turn to a simple model of calcium oscillations in sympathetic neurons from Friel et al. (1995), while this is not a cardiomyoycte, the same domains and fluxes are found, and it leads to quite a simple set of ODEs to solve.

- <a href="E10 — Exercises on Calcium Handling.ipynb#1">E10 — Exercise 1: A Simple Calcium Cycling Model</a>

<a id=serca></a>

# The SERCA pump

The model we used to explore calcium cycling in E10 is very simple, with linear currents for the Ca$^{2+}$ influx, efflux as well as uptake into the SR. These expressions work well in the certain cases to explore the system in simple terms, but more complex models can of course be built. We have already discussed L-type Ca$^{2+}$ in earlier lectures, so let us now take a closer look at the active transports carried by the NCX and SERCA, starting with the latter.

The SERCA pump, which is one of the two major actors removing calcium from the cytosol at the end of the action potential, and the primary actor refilling the SR. The name SERCA is an acronym, the full name is _sarcoplasmic-endoplasmic reticulum calcium ATPase_. The name ATPase means SERCA is a pump that uses energy stored in ATP molecules to pump calcium into the SR.

SERCA is a _high-affinity_, _high capacity_ pump. High affinity means it can maintain a high pump activity even for low cytosolic calcium concentrations. High capacity means SERCA can pump a lot of calcium at once. SERCA is high-capacity because there are _a lot_ of SERCA pumps in the cell, with much of the SR membrane being packed with SERCA molecules.

We will now look at how SERCA can be models, by stepping through from very simple to more and more complex models. The goal isn't that you need to memorize or remember all these models, but rather give some insight into how physiological models can be built with different levels of details and approximations.

But before we look at the models themselves, it is very helpful to analyze the thermodynamic limits of the SERCA pump. Please turn to the E10 notebook and solve exercise 2:

- <a href="E10 — Exercises on Calcium Handling.ipynb#2">E10 — Exercise 2: Thermodynamic Limits of SERCA</a>

Analyzing the thermodynamic limits of the pump will tell us little about kinetics or rates, but it will give insight into how large calcium gradients we expect the SERCA pump to be able to maintain.

## SERCA Pump Models

In the coming discussion we will cover a series of SERCA models in increasing complexity. This discussion is based on the following paper:

- [Koivumäki et al., _Modelling sarcoplasmic reticulum calcium ATPase and its regulation in cardiac myocytes_. Phil. Trans. R. Soc., 2009.](http://rsta.royalsocietypublishing.org/content/367/1896/2181.short)

This is a nice review that comes highly recommended if you want to do a deep dive into modeling of pumps or calcium fluxes.

#### Pumping reaction

The most basic feature of the SERCA pump is that two calcium ions bind to the pump, get transported from the cytosol into the SR, and then gets released. In general it is this process we are trying to capture, which we can write out as the simplified reaction equation

$$2{\rm Ca_i} + {\rm P} \rightleftharpoons {\rm Ca_2 P} \rightleftharpoons 2{\rm Ca_{\rm SR}} + {\rm P}.$$

Here $\rm Ca_i$ represents calcium ions in the cytosol, P represents a free SERCA pump molecule, and $\rm Ca_{\rm SR}$ represents calcium ions in the SR. There are two things worth noting from this reaction: First, the reaction, in general, _can_ happen in the reverse direction. SERCA is a bidirectional pump. In practice however, the _net_ action will typically yield a large calcium flux into the SR. The other thing you can note is that the reaction equation effectively states that the SERCA pump molecules behave like _enzymes_, as they move calcium without getting used up.

### The Unidirectional Pump Model

The simplest model of the SERCA pump we propose are based on our two observations. First we assume that the reverse activity of the pump is negligible, effectively making it a _unidirectional_ pump model. Secondly, because the pump behaves like an enzyme, we can easily model it using the Michaelis-Menten model for enzyme activity proposed in L02.

Because SERCA pumps two and two ions at a time, we get cooperativity into our model and a Hill coefficient of 2. The unidirectional pump model therefore becomes

$$J_{\rm pump} = v_{\rm max} \frac{\rm [Ca^{2+}]_{\rm i}^2}{{\rm EC}_{\rm 50}^2 + [\rm Ca^{2+}]_{\rm i}^2}.$$

The unidirectional model is very simple, both conceptually and in terms of the mathematics and implementation. This is effectively just the Hill equation. In this case the Hill coefficient is locked in at 2, and so there are two adjustable model parameters: $v_{\rm max}$ and $\rm EC_{50}$, which stands for _effective concentration_. The EC$_{50}$ is basically what we have called $K_d$, $K_m$ or $K_{1/2}$ in other cases.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/serca1.png" width=300></center>
<center><div style="max-width:600px; word-wrap:break-word;">
<b>Figure 9a:</b>
    The unidirectional SERCA pump model is one of the simplest representation of the SERCA pump. It is based on ignoring the reverse direction of the pump and treating it as a simple enzymatic process.
<br>
(Figure from Koivumäki et. al., 2009)
</div></center>
<p style="margin-bottom:1cm;"></p>

Because SERCA is a high-affinity pump, we expect EC$_{50}$ to be a small number, meaning SERCAs pumping rate should be high even for lower levels of calcium. And indeed, the best fit of this parameter to mammalian cardiomyocyte is on the order of 0.3 µM, which is a very low concentration, only slightly above the typical resting levels of calcium. This means that SERCA is working at full steam even for small elevations above the baseline. Similarly, because SERCA is high-capacity, we expect $v_{\rm max}$ to be a big number, as the total net pumping capacity is high.

Due to its simplicity, the unidirectional pump model sees quite some use. However, it does have some major issues. The biggest is that it does not care about the SR load whatsoever, in fact, this model predicts SERCA will keep pumping away calcium from the cytosol until there is nothing left. This is not only physiological, it is _unphysical_ as it breaks the law of thermodynamics, as you will see from your earlier exploration of the thermodynamics of the pump. Because of these shortcomings, models that use a unidirectional SERCA pump often have to add a phenomenological calcium leak current from the SR into the cytosol to balance the over-active SERCA. This can work in practice, but one always has to be very careful with such models, as they are not guaranteed to behave well with changing conditions.

### Unidirectional Pump with SR calcium inhibition

To rectify the fact that the unidirectional pump does not consider SR calcium at all, we could add a damping term that reduces the flux when the SR load increases. This is sometimes done. A simple implementation could be to add a simple multiplicative factor:

$$J_{\rm pump} = v_{\rm max} \frac{\rm [Ca^{2+}]_{\rm i}^2}{{\rm EC}_{\rm 50}^2 + [\rm Ca^{2+}]_{\rm i}^2} \times \frac{\rm const.}{\rm [Ca^{2+}]_{SR}}.$$

This is _somewhat_ better, as we now get a damping factor at high SR loads. The implementation is still unphysical, however, as it hasn't fixed any of the actual issue. The pump still doesn't shut of unless $\rm [Ca^{2+}]_{i}=0$ or $\rm [Ca^{2+}]_{SR}=\infty$. It is also still a unidirectional pump, as it cannot transport calcium from the SR back into the cytosol. Of course, one can make the inhibition term more complex to be more accurate, but by that point it might just be easier to make a more physical pump model.

### The Unidirectional Pump Model

The simplest model of the SERCA pump we propose are based on our two observations. First we assume that the reverse activity of the pump is negligible, effectively making it a _unidirectional_ pump model. Secondly, because the pump behaves like an enzyme, we can easily model it using the Michaelis-Menten model for enzyme activity proposed in L02.

Because SERCA pumps two and two ions at a time, we get cooperativity into our model and a Hill coefficient of 2. The unidirectional pump model therefore becomes

$$J_{\rm pump} = v_{\rm max} \frac{\rm [Ca^{2+}]_{\rm i}^2}{{\rm EC}_{\rm 50}^2 + [\rm Ca^{2+}]_{\rm i}^2}.$$

The unidirectional model is very simple, both conceptually and in terms of the mathematics and implementation. This is effectively just the Hill equation. In this case the Hill coefficient is locked in at 2, and so there are two adjustable model parameters: $v_{\rm max}$ and $\rm EC_{50}$, which stands for _effective concentration_. The EC$_{50}$ is basically what we have called $K_{\rm d}$, $K_{\rm m}$ or $K_{1/2}$ in other cases.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/serca2.png" width=300></center>
<center><div style="max-width:650px; word-wrap:break-word;">
<b>Figure 9b:</b>
    The unidirectional SERCA pump model extended by adding a SR load inhibition term.  
<br>
(Figure from Koivumäki et. al., 2009)
</div></center>
<p style="margin-bottom:1cm;"></p>

### Bidirectional Pump

Instead of trying to add the SR load dependence as a scaling factor on the current, we can do the more physiologically accurate thing and model it as a proper bidirectional pump. The simplest way to do this is to assume that the two _modes_ of the pump (i.e., the forward and reverse) are independent of each other. We can then model the two modes using our simple unidirectional pump model, i.e., using the Michaelis-Menten equation with a Hill coefficient of 2:

$$J_{\rm pump} = J_{\rm fwd} - J_{\rm rev}, \qquad J_{\rm fwd} = v_{\rm max} \frac{\rm [Ca^{2+}]_{\rm i}^2}{{\rm EC}_{\rm 50}^{{\rm fwd}\ 2} + [\rm Ca^{2+}]_{\rm i}^2}, \qquad J_{\rm rev} = v_{\rm max} \frac{\rm [Ca^{2+}]_{\rm SR}^2}{{\rm EC}_{\rm 50}^{{\rm rev}\ 2} + [\rm Ca^{2+}]_{\rm SR}^2}.$$

Here we assume the two modes has the same maximal capacity, $v_{\rm max}$, but different affinities.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/serca3.png" width=300></center>
<center><div style="max-width:650px; word-wrap:break-word;">
<b>Figure 9c:</b>
    A bidirectional SERCA pump model, treating the two modes as completely separate.
<br>
(Figure from Koivumäki et. al., 2009)
</div></center>
<p style="margin-bottom:1cm;"></p>

We can add the two terms together and use some algebra to find a single, compact expression for the net pump rate of SERCA

$$
J_{\rm pump} = v_{\rm max} \times \frac{\left(\frac{[\rm Ca^{2+}]_{\rm i}}{{\rm EC}_{\rm 50}^{{\rm fwd}}}\right)^2 - \left(\frac{[\rm Ca^{2+}]_{\rm SR}}{{\rm EC}_{\rm 50}^{{\rm rev}}}\right)^2}
{1 + \left(\frac{[\rm Ca^{2+}]_{\rm i}}{{\rm EC}_{\rm 50}^{{\rm fwd}}}\right)^2 + \left(\frac{[\rm Ca^{2+}]_{\rm SR}}{{\rm EC}_{\rm 50}^{{\rm rev}}}\right)^2}.
$$

We have effectively only added one more parameter to our model and conceptually, this model is nearly as simple as the unidirectional model. On the other hand, it turns out to be much more physiologically accurate under a wider range of conditions. First of, the pump actually shuts of when it reaches a certain SR:cytosol ratio, as we can see from solving the equation $J_{\rm pump} = 0$:

$$J_{\rm pump} = 0 \quad \Rightarrow \quad \left(\frac{[\rm Ca^{2+}]_{\rm i}}{{\rm EC}_{\rm 50}^{{\rm fwd}}}\right)^2 = \left(\frac{[\rm Ca^{2+}]_{\rm SR}}{{\rm EC}_{\rm 50}^{{\rm rev}}}\right)^2,$$

or

$$\frac{[\rm Ca^{2+}]_{\rm SR}}{[\rm Ca^{2+}]_{\rm i}} = \frac{{\rm EC}_{\rm 50}^{\rm rev}}{{\rm EC}_{\rm 50}^{\rm fwd}}.$$

Thus, by adjusting the affinity of the reverse rate of the bidirectional pump, we can get the correct baseline behavior of the SERCA model without adding the SR leak term. From our thermodynamical analysis of SERCA, we now see that the balance of the two affinities should be about 1:7000 to get the right baseline behavior.

### Buffering Capacity of SERCA

The bidirectional pump model is quite good at giving the correct pump rate into the SR, however, it does not take into effect the _buffering_ effect of SERCA. In the models we have presented so far, calcium ions get transported instaneously from the cytosol to the SR. In the model, the amount of free calcium in the cytosol and SR combined is constant.

However, in reality, what happens is that calcium ions bind to SERCA pump-molecules. They stay bound for some time, while SERCA changes conformational shape and release into the SR. However, ions can also bind, and unbind without getting transported. With the sheer amount of SERCA molecules inside the cell, this means there is, at any given time, a substantial amount of calcium ions bound to SERCA, thus SERCA is acting like a calcium _buffer_. We will discuss buffers and their effect more in the next lecture. For now, suffice to say that SERCA is the second largest calcium buffer in the cell (i.e., it binds a lot of calcium), and a detailed model should include this.

To produce a SERCA model that takes into account calcium binding, we can use a 4-state Markov model

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/serca4.png" width=300></center>
<center><div style="max-width:650px; word-wrap:break-word;">
<b>Figure 9d:</b>
    A four state Markov model of the SERCA pump. By having two states that have calcium bound to the pump, the buffering capacity of the pump is accounted for. 
<br>
(Figure from Koivumäki et. al., 2009)
</div></center>
<p style="margin-bottom:1cm;"></p>

By having four states, we have separated the process of calcium binding to SERCA, and the transport across the membrane. The states $X_2$ and $Y_2$ have calcium bound to them, while the states $X_1$ and $Y_1$ do not, thus the total amount of free calcium in the model is reduced by the amount bound to SERCA. The model pumps calcium by moving around the four states in a _reaction cycle_, first binding calcium, then transforming, then releasing the bound calcium, then transforming back. The pump velocity will be the _cycling rate_ of the model, i.e., the rate at which a full cycle of all four steps occurs. Note that the reverse mode of the pump is automatically included, as the forward and reverse modes correspond to the clockwise or counter-clockwise cycling of the model.

This model takes into account calcium bound to SERCA, and is a proper bidirectional pump that has a rate dependence on $[{\rm Ca^{2+}}]_{\rm i}$ and $[\rm Ca^{2+}]_{\rm SR}$. The biggest problem is that our model is starting to become quite complex, having 8 free parameters (the forward and reverse rates between the states), which all need to be constrained. We then also have four new states in our system of ODEs, which of course also add computational complexity. Models such as these are thus often two complex and costly to be added to full action potential models of the cell, unless someone is specifically studying SERCA dynamics in some capacity.

### Adding more complex regulation

There is no real limit to the complexitiy in which we can create Markov models of SERCA that take into account varius regulatory mechanisms. For example, SERCA depends on ATP, and we found the thermodynamic limit of SERCA activity based on this. However, so far we have not really considered the mechanism of ATP binding to SERCA and hydrolising into ADP. We have simply assumed the availability of ATP to be constant. But what if we want to include the effects of low ATP availability in energy starved cells? This could be done by adding binding and release of ATP as seperate steps in the the reaction cycle of our SERCA model.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/serca5.png" width=300></center>
<center><div style="max-width:450px; word-wrap:break-word;">
<b>Figure 9e:</b>
    A six-state markov model of the SERCA pump which takes into account the binding and hydrolysis of ATP.
<br>
(Figure from Koivumäki et. al., 2009)
</div></center>
<p style="margin-bottom:1cm;"></p>

And to take an even more extreme example, we can take a look at the 12-state SERCA model of Tran et al. (2009). As you can see from the figure, these models can become very large and complex, if one wants to capture all the detailed substates of these complex proteins. We very rarely need these kind of details unless we are specifically studying SERCA or its regulation.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/tran_model.jpg" width=600></center>
<center><div style="max-width:650px; word-wrap:break-word;">
<b>Figure 10:</b>
    The Tran model is a highly detailed SERCA model consisting of a 12-state Markov model. 
<br>
(Figure taken from <a href=https://www.ncbi.nlm.nih.gov/pubmed/19254563>Tran et al, 2009</a>.)</center>
</div></center>
<p style="margin-bottom:1cm;"></p>

# Measuring intracellular calcium dynamics

Earlier, you've heard some about how one can constrain electrophysiological models of ion channels and pumps from experimental measurements from for example patch clamp. For internal channels and pumps such as SERCA and RyR this is more complicated, because these channels only appear internally to the cell and so cannot be patch clamped. So how can we measure intracellular calcium dynamics?

One method is to add calcium indicators into the cell. These are fluorophores that bind calcium and start to glow. Using optical microscopes, we can then measure the amount of fluroesence to find an estimate of the calcium concentrations within the cell. While calcium indicators are useful, they are also quite tricky to work with, and there are limitations in both the spatial and temporal resolution. In addition, the indicators work by binding calcium, meaning you are adding a buffer into the system which buffers precisely what you are trying to measure, which will obviously impact the measurement (an example of the _observer effect_). Because of this we can also add the buffering capacity of a calcium indicators in models we make that reproduce such experiments to make it easier to compare simulation and experiment.

Another way one can get insight into the intracellular dynamics, is to measure the sarcolemmal currents which _can be_ patch-clamped, such as the NCX. Take for example measuring the SR load of a cell. If we add enough caffeine into the cell, the cell releases all its calcium into the cytosol (caffeine triggers RyR to open). In the cytosol some of this calcium gets pumped back into the SR by SERCA, but as the RyR is open, this just goes right back into the cytosol. The NCX however, is also pumping calcium out of the cell, and this efflux of calcium we can measure through a patch clamp. By integrating this current we get a good estimate of how much calcium was in the cell and SR (in total). This measurement is however, not a _dynamic_ measurement, you get a one time measurement, but you have removed all calcium from the cell itself. Effectively you are draining the entire cell of its calcium and measuring how much there is in total.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/measuring_SRload.png" width=600></center>
<center><div style="max-width:650px; word-wrap:break-word;">
<b>Figure 11:</b>
    Measuring the SR load of a cell by injecting caffeine and integrating the efflux of calcium through the NCX.  Figure taken from [Donald M. Bers Circulation Research. 2000;87:275-281].</center>
</div></center>
<p style="margin-bottom:1cm;"></p>

While this definitely is a useful experimental technique, we mainly mention it here to emphasize the challenges involved in properly measuring intracellular dynamics, and therefore how challenging it is to properly parameterize and constrain models of these phenomenon. However, the fact that these are harder to measure is also an argument for why modeling them could be more important, to reconcile our understanding of the system with the experiments we _can_ perform.

# The Sodium-Calcium Exchanger

Let us now turn to the Na$^{+}$/Ca$^{2+}$-exchanger, or NCX for short, which is the other major actor responsible for cytosolic calcium decay. Unlike SERCA, which pumps calcium into the SR, NCX pumps it fully out of the cell.

NCX is not a ATPase, or _pump_, but rather an _exchanger_. This means it manages to push calcium against its concentration gradient by allowing a co-transport of a different ion along its concentration gradient. As the name implies, NCX allows _sodium_ to enter the cell to pump calcium out.

The stoichiometry of an exchange cycle is to allow three sodium ions in, for each calcium transported out. ecause of this, the NCX is responsible for carrying two transmembrane fluxes, a calcium current, and a sodium current. Because calcium ions has a charge of $2e$, and sodium ions have a charge of $e$, there will be a net transport of charge for each NCX cycle, so the NCX is not only responsible for ion transport, but will also change the membrane potential. The NCX is therefore not only important for calcium handling, but also for properly modeling the action potential.

### Modeling NCX with Michaelis-Menten kinetics

The simplest model we had for SERCA used the Michaelis-Menten equation, modeling the activity as increasing with the cytosolic calcium level, eventually becoming saturated. Experiments show that the NCX current exhibits similar saturation behavior with the cytosolic calcium. We can therefore propose using a similar model for NCX:
$$I_{\rm NCX} = v_{\rm max} \frac{\rm [Ca^{2+}]_{\rm i}}{{\rm EC}_{\rm 50} + [\rm Ca^{2+}]_{\rm i}},$$
This model describes the dependence on cytosolic concentration and the saturation behavior quite well, but it has some serious limitations, as it completely neglects the impact the cytosolic sodium levels and the membrane potential play on the NCX. This can be remedied by letting the parameters $v_{\rm max}$ and $\rm EC_{\rm 50}$ be functions of $V$ and $\rm [Na^{2+}]_i$, rather than be constant, but this is a significant increase in complexity.

Put simply, the fact that there is a potential across the cell membrane, and the co-transport of sodium, makes NCX a lot more complex than SERCA. To better understand the effect $V$ and $\rm [Na^{2+}]_i$ has on NCX activity, we can start by analyzing the thermodynamics of the NCX. You can do this by turning to E10 and solving the following exercise:

- [E10 — Exercise 3: Thermodynamics of the NCX](E10%20—%20Exercises%20on%20Calcium%20Handling.ipynb#3)

#### An ohmic NCX current?

In the exercise, you analyzed the thermodynamics of NCX. This is interesting because the exchanger couples to _passive_ transports together, but the NCX as a whole is then dependent on the net change in Gibbs energy of the two transports. From this we found a _reversial potential_ of the NCX to be

$$E_{\rm NCX} = 3E_{\rm Na} - 2E_{\rm Ca}.$$

This reversal potential is completely analogous to the reversal potential (aka the Nernst potential) of our other passive ionic currents. Put simply it is the potential at which the NCX activity ceases, because there is no electrodiffusive driving force. Below this potential NCX drives calcium out of the cell, and above it lets calcium into the cell.

Because we found a neat reversial potential that is analogous to our Nernst potentials, it might be tempting to model the NCX as a simple Ohmic current (and then compute the Na$^{2+}$ and Ca$^{2+}$ fluxes from this

$$I_{\rm NCX} = g_{\rm NCX}(V - E_{\rm NCX})$$.

However, recall that the ohmic current was already a pretty rough approximation even for individual ion currents. In practice, the NCX is a bit too complex to approximate in an ohmic manner, and the kinetics of the current will depend strongly on the calcium and sodium concentrations. However, the reversal potential does tell us something about when the exchanger is active, and in which direction the exchange is occurring, so the analysis was useful even if it does not give a direct current expression.

### A more realistic formulation of the NCX

If you recall back to our discussion on the ohmic approximation, we mention that there is a more advanced form one can use, called the Goldman-Hodgkin-Katz flux equations. This is a good starting point to find a better NCX flux equation. However, in the interest of time, we won't dive into these derivations now, but rather give a quick historical walk through and simply show you the expression, which really has become a beast.

This formulation was first proposed by Mullins (1977), then simplified by DiFrancesco and Noble (1985), and then further modified by Luo & Rudy (1994), which resulted in this expression

$$
I_{\rm NaCa} = k_{\rm NaCa}
\frac{
{\rm[Na^+]^3_i [Ca^{2+}]_e} \cdot {\rm exp}\left[\eta\cdot V\cdot\frac{F}{RT}\right]
- {\rm[Na^+]^3_e [Ca^{2+}]_i} \cdot {\rm exp}\left[(\eta-1)\cdot V\cdot\frac{F}{RT}\right]
}{
\left(K_{\rm m, Na}^3 + [{\rm Na}^+]_{\rm e}^3\right)
\left(K_{\rm m, Ca} + [{\rm Ca}^{2+}]_{\rm e}\right)
\left(1 + k_{\rm sat}\cdot {\rm exp}\left[(\eta-1)\cdot V\cdot\frac{F}{RT}\right]\right)}
$$

This expression is, conceptually, really heavy. It is computationally speaking also a bit costly, at least compared to a linear expresison, or simple Hill equation, because of all the exponential calls (which are a little expensive). However, it doesn't actually add any new states to our models, which is nice.

Note also that it doesn't actually have that many parameters. It depends on the membrane potential and intra- and extracellular concentrations, but other than that there are only five actual model parameters, which are $k_{\rm NaCa} = 2000$ µA/µF, $K_{\rm m,Na} = 87.5$ mmol/L, $K_{\rm m,Ca} = 1.38$ mmol/L, $k_{\rm sat} =0.1$ and $\eta = 0.35$.

While this flux is quite intense, let us briefly look at how it actually behaves in practice. Below we have given a figure from the book _Excitation-Contraction Coupling and Cardiac Contractile Force_ by Donald M. Bers.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/bers.png" width=600></center>
<center><div style="max-width:650px; word-wrap:break-word;">
<b>Figure 11:</b>
    Behavior of the Luo-Rudy (1994) NCX formuilation. Note that $E_{\rm m}$ is the membrane potential, while $E_{\rm Na/Ca}$ is the reversal potential of the NCX.</center>
</div></center>
<p style="margin-bottom:1cm;"></p>

### NCX and Afterdepolarization

The fact that the NCX is electrogenic when it is pumping calcium also makes it a potential candidate for arrythmeogenic behavior of the cell, i.e., abnormal electrical behavior. If the cytosolic calcium concentration of the cell is too high at rest, the NCX will be active and pump calcium out of the cell, however, because three sodium ions are let in for each calcium that is pumped out, the NCX carries an inward current that is depolarizing the cell. Under normal conditions, this occurs during the repolarizing phase of the action potential, and the inward current is more than accounted for by other outward currents, such as potassium. In abnormal cases, however, the NCX can potentially be strong enough to depolarize the cell enough to trigger a new action potential. A sudden and spontaneous release of calcium from the SR while the cell is at rest for example, could potentially trigger an AP. And such spontaneous releases _do_ occur (typically called _calcium waves_). The NCX is therefore an important actor in arrythemogenesis and disease.

<a id="regulation"></a>

## Regulation of Excitation-Contraction coupling

There are several mechanisms built into the cell to regulate excitation-contraction coupling and in that manner change the force of contraction. It depends on the complexity and purpose of our cell models whether we want to incorporate such regulatory mechanisms. Let us look at a few of these mechanisms.

### Regulation of SERCA

As we discussed earlier, the balance between SERCA and NCX is an important factor in EC coupling, because this balance is what decides the SR load of the cell, which in turn shifts the calcium transient. One direct way of regulating EC coupling is thus to up- or down-regulate SERCA. SERCA is regulated by a molecule called _phospholamban_ (PLB), which inhibits SERCA activity. You can think of PLB as the brakes on SERCA, and under normal conditions, these "brakes" are partially on.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/serca_plb.png" width=300></center>
<center><div style="max-width:650px; word-wrap:break-word;">
<b>Figure 12:</b>
The SERCA-pump is one of the main actors in EC coupling, and up- or down-regulating SERCA is a major way of shifting the EC coupling balance. SERCA is regulated directly by phospholamban (PLB).
</div></center>
<p style="margin-bottom:1cm;"></p>

There are two important signaling molecules that both downregulate the activity of PLB, these are Protein Kinase A (PKA) and CaMKII. When PLB is downregulated, the "brakes" get taken of and SERCA activity is increased. As a result, SR load increases giving stronger calcium transients and stronger contraction.

### Beta-adrenergic stimulus

Both PKA and CaMKII are involved in several signaling cascades in the body, but perhaps the most important one is the beta-adrenergic response that occurs when $\beta$-receptors on the outside of the cell react to epinephrine (adrenaline) or norepinephrine. When this happens ATP gets converted to cyclic AMP (cAMP) which in turn activates PKA and CaMKII starting many different signaling pathways, including upregulating SERCA. In addition $\beta$-adrenergic stimulus increases the amount of calcium that enters through L-type channels, and make myofilaments more sensitive to calcium. These three changes combine to give a much strong calcium transient.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/beta_adrenergic.png" width=600></center>
<center><div style="max-width:650px; word-wrap:break-word;">
<b>Figure 13:</b>
Schematic of the main targets of $\beta$-adrenergic stimulus (epinephrine/adrenaline) in the cardiomyocyte. Most important for calcium handling is that PKA and CaMKII activate, which upregulate SERCA, L-type calcium and the calcium sensitivity of the myofilaments.
</div></center>
<p style="margin-bottom:1cm;"></p>

When the heart receives epinephrine, both the heart rate and the force of contraction increases. This leads to what we call a _positive force-frequency_ relationship, meaning the faster the heart beats, the stronger each beat is. This relationship is shown in the Figure below. The white symbols are a healthy heart, and as the frequency increases so does the SR load and the produced force. The black symbols however, show a failing heart, and you can see there is no longer a proper positive force-frequency relationship.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/force_frequency.png" width=400></center>
<center><div style="max-width:750px; word-wrap:break-word;">
<b>Figure 14:</b>
A healthy heart has a positive force-frequency relationship, meaning the faster the heart rhythm, the stronger the heart beat. Figure taken from [Donald M. Bers Circulation Research. 2000;87:275-281]
</div></center>
<p style="margin-bottom:1cm;"></p>

In the models we covered of SERCA, we did not include direct regulation from PLB, PKA or CaMKII, but these effects might be important to include if you want your model to include, $\beta$-adrenergic response, heart failure, or the effects of drugs. One fairly simple such model has been proposed by [Koivumäki et al](http://rsta.royalsocietypublishing.org/content/367/1896/2181.short).

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/serca_regulation_model.png" width=300></center>
<center><div style="max-width:450px; word-wrap:break-word;">
<b>Figure 15:</b>
A simple SERCA model proposed by Koivumäki et al., that includes regulation by PKA/CaMKII via PLB.
</div></center>
<p style="margin-bottom:1cm;"></p>

### Auto-regulation

As a last example to discuss for intracellular calcium handling for this time, let us look at how cardiomyoyctes to a certain extent as _self-regulating_. Below we show some figures taken from some experiments performed at the institute of experimental medical research at Oslo university hospital.

In this experiment they are pacing a cardiomyocyte at a regular pace and measuring the amount of shortening per beat. The shortening is a measure of the strength of contraction, and thus a function of the magnitude of the calcium transient. When pacing at a regular pace, we observe the shortening is fairly constant. This is because the cell is in a steady cycle, and thus the transients produced per action potential are roughly the same magnitudes. But what happens when a bit of caffine is added?

#### Adding caffine

Caffine binds to the calcium release channels (RyR), increasing their calcium sensitivity, making them more likely to open. This in turn gives a bigger calcium release and therefore a bigger transient. This response is very fast, as we can see from the trace.

However, after caffine has been added, the system is no longer in a steady cycle, but adjusting every consecutive beat. What we witness is that while the beats do indeed lead to a bigger shortening, the _increase_ is decaying for every beat with the shortening slowly reverting to the pre-caffine levels. This is not due to any manipulations from our side, it is a pure autoregulation.

Trying to think through what is happening can now be a very good summary of todays lecture.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/autoregulation0.png" width=500></center>
<center><div style="max-width:550px; word-wrap:break-word;">
<b>Figure 16:</b>
A cardiomyocyte is paced at a regular interval, and the resulting cell shortening is measured as a proxy of strength of contraction. Caffine is then added to the cell to sensitize RyR, giving a stronger calcium release. We see that this gives a stronger contraction in the short term, but over a series of beats the cell gradually autoregulates to the same baseline. The reverse situation happens once caffine gets taken off the system again. [Adapted from Sjaastad et al., 2003]
</div></center>
<p style="margin-bottom:1cm;"></p>

To understand _why_ the system autoregulates to the previous baseline, you need to consider where calcium is coming from and going. The caffine increased the RyR-mediated release calcium, but the balance of NCX and SERCA is _not_ shifted. This means some of the additional calcium released from the SR is pumped out of the cell. While caffine increases the strength of contraction, it does so at the expense of SR load. As SR load drops, so does the transient. Put simply, the system with added caffine is not in equilibrium, and so over time it drifts back to its old magnitudes, albeit with a lower overall SR load.

This is also why the calcium transient becomes much smaller when we take of the caffine again. In that case the RyR are no longer sensitized, but we still have a low SR load. The SR release is therefore quite weak and the shortening become small. However, because little calcium is released from the SR, more calcium is entering through the L-type in comparison, and so the SR load is gradually refilling over time until the system has drifted back to the old equilibrium.

#### Adding tetracaine

The same authors carried out the same experiment, but adding _tetracaine_ instead of caffine, which has the opposite effect on RyR, it inhibits release. Right after adding tetracaine to the system, the calcium release is much weaker, giving a small calcium transient and little shortening. However, over the next 10–20 beats, the cell is auto-regulating by increasing its SR load, until we are back to nearly the same baseline as we started at. Taking tetracaine off the system gives the same behavior as when removing caffine, the increased SR load gives a larger shortening, but after some time the SR load has decreased back to its original value.

In this experiment they also measured both the L-type current and the NCX current of the cell, which when summed gives us a measure of the total amount of calcium entering and leaving the cell. We see that this measurement at baseline goes to 0 over one beat (measured points at a and c). This means there is no net calcium entering or leaving the cell, and thus the SR load is constant. However, after perturbing the system, we see that the integral does _not_ go to 0. This means a net amount of calcium is either entering or leaving the cell (entering at point b, and leaving at point d). This calcium cannot be in the cytosol, so it has to end up in the SR.

<p style="margin-bottom:1cm;"></p>
<center><img src="fig/tetracaine.png" width=500></center>
<center><div style="max-width:550px; word-wrap:break-word;">
<b>Figure 16b:</b>
Adding Tetracaine to the cardiomyocyte inhibits SR calcium release, giving a smaller overall calcium transient. But due to an increasing SR load, the cell autoregulates back to the same baseline contraction. The experimentalists have also added patch-clamp data from L-type and NCX to find the net calcium transport into/out of the cell. This is a good confirmation that our explanation of intracellular calcium handling is a reasonable explanation of the observed cell behavior. <br>
[Adapted from Sjaastad et al., 2003]
</div></center>
<p style="margin-bottom:1cm;"></p>

While these experiments are quite specific, they illustrate quite nicely how complicated biological systems can auto-regulate in response to perturbations, and in that sense become more stable. In our case, these experiments also served as a nice summary of our discussions on intracellular calcium handling.
